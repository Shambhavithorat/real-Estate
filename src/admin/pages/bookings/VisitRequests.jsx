...
const VisitRequests = () => {
  const visits = [
    ...
    { id: 3, name: 'David Smith', property: 'Modern Oasis', date: '14 May 2026', time: '11:30 AM', status: 'Rejected' },
  ];
    ...
    <div className="space-y-10 pb-12 relative min-h-screen bg-[#FBFBFB] -m-4 p-8 rounded-[40px]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8 px-2">
        <div className="space-y-1">
          <h1 className="text-5xl font-black text-[#111111] tracking-tighter">Visit <span className="text-[#6B705C] italic font-serif">Requests</span></h1>
          <p className="text-sm text-[#666666] font-medium tracking-tight">Manage physical and virtual property tours.</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-[#E5E5E5] bg-white">
              {visits.map(v => (
                <tr key={v.id} className="hover:bg-[#F7F7F5] transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest rounded-full ${v.status === 'Approved' ? 'bg-green-100 text-green-700' : v.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                      {v.status}
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    {v.status === 'Pending' && (
                        <>
                           <button className="text-green-600 hover:text-green-800 text-sm font-bold mr-4 transition-colors">Approve</button>
                           <button className="text-red-500 hover:text-red-700 text-sm font-bold transition-colors">Reject</button>
                        </>
                    )}
                    {v.status !== 'Pending' && (
                       <button className="text-[#6B705C] hover:text-[#111111] text-sm font-bold transition-colors">Details</button>
                    )}
                  </td>
                </tr>
              ))}
